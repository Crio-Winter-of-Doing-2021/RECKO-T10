...
from .serializers import EmptySerializer, XeroSerializer, XNestedSerializer1, QuickBooksSerializer,QuickBooksSerializer1, XNestedSerializer3,QNestedSerializer1,QNestedSerializer2,QNestedSerializer3,QNestedSerializer4
...
def xeroDataEntry(response):
    ...
    #Entry.objects.filter(name='name', title='title').exists() to check if entry is present or not
    for obj in response['Journals']:
        list1 = []
        #print("Date: ",obj['JournalDate'])
        dt = datetime.fromtimestamp(int(obj['JournalDate'][6:-10]))#datetime.utcfromtimestamp(int(obj['JournalDate'][6:-7]))
        ...
        for journalLine in obj['JournalLines']:
                ...
                if s1.is_valid():
                    ...
                    if Accounts.objects.filter(accountName=accountName, accountId=accountId,amount=amount,date=date,accountType=accType,providerName="Xero").exists() is False:
                        record=Accounts(accountName=accountName, accountId=accountId,amount=amount,date=date,accountType=accType,providerName="Xero")
                        record.save()
